...
% 
%     ** These structure fields are mandatory to use with PlaceStructure **
% 
...
[layer, dtype, w, phaseShift, period, corw, dc, chirp] = NumberOfColumns(cols, layer, dtype, w, phaseShift, period, corw, dc, chirp);


%% Bragg structure definition
for row = 1 : rows
  cols = size(layer, 2);
    ...
    if(obragg(row).corw(col) > 0)
      ...
      if(~isempty(obragg(row).phaseShift))
        if(~isempty(obragg(row).phaseShift(col).pos))
          tps = zeros(periods,1); 
          psCount = max([length(obragg(row).phaseShift(col).pos) length(obragg(row).phaseShift(col).value)]);
          [obragg(row).phaseShift(col).pos, obragg(row).phaseShift(col).value] = NumberOfColumns(psCount, obragg(row).phaseShift(col).pos, obragg(row).phaseShift(col).value);

          for psIndex = 1 : psCount
             tindex = find(x1 >= obragg(row).phaseShift(col).pos(psIndex), 1, 'first');
             tps(tindex) = tps(tindex) + obragg(row).phaseShift(col).value(psIndex) * obragg(row).period(col) / (2 * pi);
          end
          x1 = x1 + cumsum(tps);
        end
      end
